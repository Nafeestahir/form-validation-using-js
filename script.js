...
function validateForm() {
    ...
    let letters = /^[A-Za-z]+$/;
    let num = /.*(?=.*\d)(?=.*[a-zA-Z])(?=.*['.(self::REGEX_SPECIAL_CHARS).']).*/;
    let numeric = /[0-9]+$/;
    let email = /^\S+@\S+\.\S+$/;
    ...
    if (!lname.match(letters)) {
        seterror("lname", "*Only alphabets are allowed");
        returnval = false;
    }

    let pass = document.forms["myform"]["password"].value;
    if (!pass.match(num)) {
        seterror("Passerr", "*Write atleast one letter,number and special character");
        returnval = false;
    }
    ...
    if (!Phone.match(numeric)) {
        seterror("phoneerr", "*Only Numbers are allowed");
        returnval = false;
    }
    let Age = document.forms["myform"]["age"].value;
    if (Age=="") {
        seterror("ageerr", "*Please enter your Age");
        returnval = false;
    }
    let Location = document.forms["myform"]["location"].value;
    if (Location == "") {
        seterror("locationerr", "*Please enter your Location");
        returnval = false;
    }
    return returnval;
